    ...
    return <div className="container mt-24 flex mx-auto gap-8 flex-col items-center">
		...
		<div className='flex items-center flex-col z-20 w-5/6 xl:w-1/2 gap-12 xl:gap-8'>
			<Form className='flex w-full xl:w-1/2  items-center gap-8 z-20 flex-col' method='POST'>
				...
				<button className='w-4/5 rounded-lg bg-slate-800 px-6 py-2 text-white hover:opacity-70 duration-300' ><span>Login</span></button>
			</Form>
			<hr className='w-full' />
			<span className='text-4xl xl:text-lg'>Don't have an account? <Link to={'/register'} prefetch='intent' className='font-bold underline'>Register</Link> </span>
		</div>
	</div>
}
